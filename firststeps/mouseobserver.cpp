...
QString MouseObserver::modifiersInfo(QMouseEvent *pe)
{
    ...
    if(pe->modifiers() & Qt::ShiftModifier) strModifiers += "Shift ";
    if(pe->modifiers() & Qt::ControlModifier) strModifiers += "Control ";
    if(pe->modifiers() & Qt::AltModifier) strModifiers += "Alt ";
    return strModifiers;
}
...
QString MouseObserver::buttonsInfo(QMouseEvent *pe)
{
    ...
    if(pe->buttons() & Qt::RightButton) strButtons += "Right ";
    if(pe->buttons() & Qt::MidButton) strButtons += "Middle ";
    return strButtons;
}
