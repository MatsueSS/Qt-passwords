...
/*virtual*/void PasswordFileManager::read(const string& filename)
{
    ...
    if(file.open(QIODevice::ReadOnly | QIODevice::Text)){
        ...
        while(!in.atEnd()){
            ...
            password->setName(line[0]);
            password->setPassword(line[1]);
        }
    }
...
/*virtual*/void PasswordFileManager::write(const string& filename)
{
    ...
    if(file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)){
        ...
        out << password->getName() << " " << password->getPassword() << "\n";
    }
    else { QErrorMessage err; err.showMessage("File cannot be opened"); }
    file.close();
}
...
const QMap<string, Password>& LibPasswords::getLib(){
    return libPsw;
}
...
/*virtual*/void LibPasswordsFileManager::write(const string& filename){
    ...
    if(file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)){
        QTextStream out(&file);
        QMap<string, Password> psw = lib->getLib();
        QMap<string, Password>::iterator it = psw.begin();
        while(it != psw.end()){
            out << it.key() << " " << it.value().getPassword() << "\n";
        }
    }
    else { QErrorMessage err; err.showMessage("File cannot be opened"); }
    file.close();
